...
export const getImage = (idProducto: string): Promise<any> => {
  return new Promise(async (resolve, reject) => {
    try {
      const existenciaProducto = await verifyExistProduct(idProducto);
      if (existenciaProducto) {
        const queryObtenerImagen = "SELECT image_product FROM product WHERE id_product = ?";
        connection.query(queryObtenerImagen, idProducto, (error: any, results: any) => {
            if (error) {
              reject(error);
            } else {
              resolve(results);
...
export const updateDataProdcut = (data: any): Promise<any> => {
    return new Promise((resolve, reject) => {
